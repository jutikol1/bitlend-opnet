...
import { CreateWalletParams, GetBlockTemplateParams, GetChainTxStatsParams, ImportDescriptorsParams, PsbtBumpFeeParams, RPCIniOptions, SendAllParams, SendParams, SendRawTransactionParams } from './external/rpc.js';
...
import { BestBlockHeader, ChainLockVerification, DeploymentInfo, GenerateBlockResult, HDKeyInfo, ImportDescriptorsResult, IndexInfo, ListDescriptorsResult, MempoolEntryById, MigrateWalletResult, NodeAddress, PackageResult, PeerAddressResult, PsbtBumpFeeResult, SendAllResult, SendResult, SimulateTransactionResult, TestMempoolAcceptResult, UpgradeWalletResult, WalletDescriptor } from './types/NewMethods.js';
...
export declare class BitcoinRPC extends Logger {
    ...
    getBestBlockHeader(verbose?: boolean): Promise<BestBlockHeader | string | null>;
    getDeploymentInfo(blockHash?: string): Promise<DeploymentInfo | null>;
    getMempoolEntryById(wtxid: string): Promise<MempoolEntryById | null>;
    generate(nBlocks: number, maxTries?: number, wallet?: string): Promise<string[] | null>;
    generateBlock(output: string, transactions?: string[], submit?: boolean): Promise<GenerateBlockResult | null>;
    addPeerAddress(address: string, port: number, tried?: boolean): Promise<PeerAddressResult | null>;
    getNodeAddresses(count?: number): Promise<NodeAddress[] | null>;
    sendMsgToPeer(peerId: number, msgType: string, data: string): Promise<void>;
    submitPackage(packageTxs: string[], maxFeeRate?: number | string, maxBurnAmount?: number | string): Promise<PackageResult | null>;
    getIndexInfo(indexName?: string): Promise<IndexInfo | null>;
    verifyChainLock(blockHash: string, signature: string, height: number): Promise<ChainLockVerification | null>;
    createWalletDescriptor(type: 'wpkh' | 'pkh' | 'sh' | 'tr', internal?: boolean, hdkey?: string, wallet?: string): Promise<WalletDescriptor | null>;
    getHDKeys(activeOnly?: boolean, privateKeys?: boolean, wallet?: string): Promise<HDKeyInfo[] | null>;
    importDescriptors(requests: ImportDescriptorsParams['requests'], wallet?: string): Promise<ImportDescriptorsResult | null>;
    listDescriptors(includePrivate?: boolean, wallet?: string): Promise<ListDescriptorsResult | null>;
    migrateWallet(walletName?: string, passphrase?: string): Promise<MigrateWalletResult | null>;
    newKeypool(wallet?: string): Promise<void>;
    getBlockTemplate(params: GetBlockTemplateParams): Promise<BlockTemplate | null>;
    submitBlock(blockHex: string): Promise<string | null>;
    psbtBumpFee(txid: string, options?: PsbtBumpFeeParams['options'], wallet?: string): Promise<PsbtBumpFeeResult | null>;
    send(outputs: SendParams['outputs'], options?: {
        confTarget?: number;
        estimateMode?: 'UNSET' | 'ECONOMICAL' | 'CONSERVATIVE';
        feeRate?: number | string;
        advancedOptions?: SendParams['options'];
    }, wallet?: string): Promise<SendResult | null>;
    sendAll(recipients: SendAllParams['recipients'], options?: {
        confTarget?: number;
        estimateMode?: 'UNSET' | 'ECONOMICAL' | 'CONSERVATIVE';
        feeRate?: number | string;
        advancedOptions?: SendAllParams['options'];
    }, wallet?: string): Promise<SendAllResult | null>;
    simulateRawTransaction(rawTxs?: string[], includeWatchOnly?: boolean, wallet?: string): Promise<SimulateTransactionResult | null>;
    upgradeWallet(version?: number, wallet?: string): Promise<UpgradeWalletResult | null>;
    ...
    testMempoolAccept(rawtxs: string[], maxfeerate?: number | string): Promise<TestMempoolAcceptResult[]>;
    decodeRawTransaction(hexString: string, isWitness?: boolean): Promise<IRawTransaction | null>;
