...
import { DEFAULT_CONFIG } from './websocket/types/WebSocketClientConfig.js';
import { InternalError } from './websocket/types/WebSocketErrorCodes.js';
import { WebSocketRequestOpcode, WebSocketResponseOpcode, } from './websocket/types/WebSocketOpcodes.js';
import { WebSocketClientEvent, } from './websocket/types/WebSocketProviderTypes.js';
export class WebSocketRpcProvider extends AbstractRpcProvider {
    ...
    async subscribeMempool(handler) {
        ...
        const encodedPayload = type.encode(message).finish();
        const requestId = this.nextRequestId();
        const fullMessage = this.buildMessage(WebSocketRequestOpcode.SUBSCRIBE_MEMPOOL, requestId, encodedPayload);
        await this.sendRequest(requestId, fullMessage);
        ...
        for (const [type, handler] of handlers) {
            try {
                if (type === SubscriptionType.BLOCKS) {
                    ...
                    await this.subscribeMempool(handler);
                }
            }
            catch (e) {
